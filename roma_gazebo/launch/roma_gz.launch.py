...
ARGUMENTS = [
    DeclareLaunchArgument('world_name', default_value='flat_world.sdf'),
    DeclareLaunchArgument('ros_bridge', default_value='True'),
    DeclareLaunchArgument('initial_pose_x', default_value='0.0'),
    DeclareLaunchArgument('initial_pose_y', default_value='0.0'),
    DeclareLaunchArgument('initial_pose_z', default_value='0.15'),
    ...
    DeclareLaunchArgument('entity', default_value='roma'),
]

def get_roma_description():
    pkg_roma_gazebo = get_package_share_directory('roma_gazebo')
    pkg_roma_description = get_package_share_directory('roma_description')
    robot_description_path = os.path.join(pkg_roma_gazebo, 'urdf', 'roma_gz.urdf.xacro')
    mappings = {}
    ...
    robot_desc = robot_desc.replace(
        'package://roma_description/', f'file://{pkg_roma_description}/'
    )
    return robot_desc
...
def generate_launch_description():
    ...
    world_path = os.path.join(pkg_roma_gazebo, 'worlds', 'flat_world.sdf')
    bridge_config_file_path = os.path.join(pkg_roma_gazebo, 'config', 'bridge_config.yaml')
            ...
            parameters=[
                {
                    ...
                    'robot_description': ParameterValue(get_roma_description(), value_type=str),
                }
            ],
            remappings=[
                ('/tf', 'tf'),
